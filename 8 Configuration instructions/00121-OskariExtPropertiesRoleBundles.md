...
```
Here the `actionhandler.GetAppSetup.dynamic.bundles` refers to bundles and `actionhandler.GetAppSetup.dynamic.bundle.[BUNDLE ID].roles` is used to set comma-separated list of roles that will have the bundle added to the app setup even when not part of it in the database. For example admin-bundles are added like this, but it can be used for any bundle.
 Note! If the user has more than one role that is listed, the bundle will be started multiple times.
