...
```javascript
import 'oskari-bundle!oskari-frontend/bundles/statistics/statsgrid';
```
...
Note that you need to change the package to match your applications module (in the sample-server-extension the module is called `app` and it's located here: https://github.com/oskariorg/sample-server-extension/tree/master/app-resources/src/main/java/flyway/app). You will also need to change the version number (in the class name) to match your application. Picking the next number from the current version is ok. If this is your first migration for the app you will also need to define the module to be activated in `oskari-ext.properties`:
