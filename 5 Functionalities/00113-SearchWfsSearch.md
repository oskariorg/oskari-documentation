...
```
The frontend code for getting the administrative user interface is provided by `admin-wfs-search-channel` in the `tampere` namespace [here](https://github.com/oskariorg/oskari-frontend-contrib/tree/master/bundles/tampere/admin-wfs-search-channel). You can add configure it as an admin bundles in `oskari-ext.properties` by adding it to the dynamic bundles list:
