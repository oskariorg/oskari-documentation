...
done automatically with migration scripts using https://flywaydb.org/.
...
The upgrade is done with a modular setup with these default modules:   
 ...
 Oskari-based server-applications can also use a migration module to initialize the database content and applications setups.
 For example the sample-server-extension uses the `app` module to initialize a couple dataproviders, default users, an application specific bundle, application setups (collection of bundles to be shown as geoportal for the user) and layers to the database as content.
...
- SQL: https://github.com/oskariorg/sample-server-extension/tree/master/app-resources/src/main/resources/flyway/app
- Java: https://github.com/oskariorg/sample-server-extension/tree/master/app-resources/src/main/java/flyway/app
...
Oskari logs a message (with log level `info`) `"Oskari-map checking DB status"` when the migration starts. When the core
 ...
 on error is the only tool that can be provided.
 
 Commonly known issues are listed in [Writing upgrade scripts](00041-AddingDatabaseMigrations.md) document.
...
Otherwise Flyway will log it as an error. Instructions for writing scripts can be found in the
 [Writing upgrade scripts](00041-AddingDatabaseMigrations.md) document.
...
In the Arctic-SDI geoportal, myplaces or userlayers are not needed so `oskari-ext.properties` defines only the application specific module.
This means that the database structure for myplaces and userlayers are not created when starting the application:
...
Arctic-SDI geoportal also has an example for modifying the appsetups when a new functionality (frontend bundle) is added or zoomlevels are tuned:
